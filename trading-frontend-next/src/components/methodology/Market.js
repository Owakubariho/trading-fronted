...
const Market = () => {
  ...
  <p className="text-lg font-bold text-gray-800 mb-2">Commitment of traders-report<strong> <span className='text-blue-600'>indicators on trading view</span></strong></p>
  <ul className="list-disc list-inside text-gray-700">
    <li className='lowercase'>NON COMM INDEX</li>
    <li className='lowercase'>COMM_INDEX_NET</li>
    <li className='lowercase'>SPEC _TURNS</li>
  </ul>
  <div>
    ...
    {indicators.map((indicator, index) => (
            <li className="py-1 px-2 bg-gray-100 rounded-md mb-1 " key={index}>
              <span className="uppercase text-blue-600">{indicator.name}</span>: {indicator.description}
            </li>
          ))}
    </ul>
  </div>
