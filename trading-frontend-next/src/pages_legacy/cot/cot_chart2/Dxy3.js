...
export const Dxy3 = () => {
  ...
  const getStock = async () => {
    const response = await fetch("http://127.0.0.1:8000/Dxy/");
  ...
  // dxy
  const dxy = data.find((item) => item.Asset === "USD INDEX");
  console.log(dxy);
  // Output:
  // {
  ...
  // }
  const commercialIndex = dxy?.CommercialWilliamsIndex;
  console.log(commercialIndex); // "71.41"
  const nonCommercialIndex = dxy?.NonCommercialWilliamsIndex;
        ...
        <div className="relative h-96">
          <Line
            ...
            options={{
              ...
              plugins: {
                title: {
                  text: "USD INDEX - ICE FUTURES U.S.",
                },
              },
              scales: {
                x: {
