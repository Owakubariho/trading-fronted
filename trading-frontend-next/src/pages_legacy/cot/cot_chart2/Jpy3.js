...
export const Jpy3 = () => {
  ...
  const getStock = async () => {
    const response = await fetch("http://127.0.0.1:8000/Jpy/");
  ...
  const JAPANESEYEN = data.find((item) => item.Asset === "JAPANESE YEN");
  ...
  // }
  const commercialIndex = JAPANESEYEN?.CommercialWilliamsIndex;
  console.log(commercialIndex); // "71.41"
  const nonCommercialIndex = JAPANESEYEN?.NonCommercialWilliamsIndex;
        ...
        <div className="relative h-96">
          <Line
            ...
            options={{
              ...
              plugins: {
                title: {
                  text: "JAPANESE YEN - CHICAGO MERCANTILE EXCHANGE",
                },
              },
              scales: {
                x: {
