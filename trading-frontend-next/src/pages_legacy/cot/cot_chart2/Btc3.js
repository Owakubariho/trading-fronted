...
export const Btc3 = () => {
  ...
  const getStock = async () => {
    const response = await fetch("http://127.0.0.1:8000/Btc/");
  ...
  const bitcoin = data.find((item) => item.Asset === "BITCOIN");
  console.log(bitcoin);
  // Output:
  // {
  ...
  // }
  const commercialIndex = bitcoin?.CommercialWilliamsIndex;
  console.log(commercialIndex); // "71.41"
  const nonCommercialIndex = bitcoin?.NonCommercialWilliamsIndex;
        ...
        <div className="relative h-96">
          <Line
            ...
            options={{
              ...
              plugins: {
                title: {
                  text: "BITCOIN - CHICAGO MERCANTILE EXCHANGE",
                },
              },
              scales: {
                x: {
