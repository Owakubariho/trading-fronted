...
function Minerviniexness() {
    let [stock, SetStock]=useState([])
      ...
      let getStock= async () =>{
        const response = await fetch('http://127.0.0.1:8000/minerviniexness/');
              ...
              <table className="min-w-full text-left text-sm font-light">
                  ...
                  {stock.map((item, index) => (
                    <tr  key={index}  className="border-b bg-neutral-100 dark:border-neutral-500 dark:bg-neutral-700">
                      ...
                      <td className="whitespace-nowrap px-6 py-4">{item.sector}</td>
...
export default Minerviniexness
