...
export const Eur3 = () => {
  ...
  const getStock = async () => {
    const response = await fetch("http://127.0.0.1:8000/Eur/");
  ...
  const euro = data.find((item) => item.Asset === "EURO FX");
  console.log(euro);
  // Output:
  // { euro: "EURO FX",
  ...
  // }
  const commercialIndex = euro?.CommercialWilliamsIndex;
  console.log(commercialIndex); // "71.41"
  const nonCommercialIndex = euro?.NonCommercialWilliamsIndex;
        ...
        <div className="relative h-96">
          <Line
            ...
            options={{
              ...
              plugins: {
                title: {
                  text: "EURO FX - CHICAGO MERCANTILE EXCHANGE",
                },
              },
              scales: {
                x: {
