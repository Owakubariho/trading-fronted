...
export const Copper3 = () => {
  ...
  const getStock = async () => {
    const response = await fetch("http://127.0.0.1:8000/Copper/");
  ...
  const COPPER = data.find((item) => item.Asset === "COPPER");
  console.log(COPPER);
  // Output:
  // {
  ...
  // }
  const commercialIndex = COPPER?.CommercialWilliamsIndex;
  console.log(commercialIndex); // "71.41"
  const nonCommercialIndex = COPPER?.NonCommercialWilliamsIndex;
        ...
        <div className="relative h-96">
          <Line
            ...
            options={{
              ...
              plugins: {
                title: {
                  text: "COPPER- #1 - COMMODITY EXCHANGE INC.",
                },
              },
              scales: {
                x: {
