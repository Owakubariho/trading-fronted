...
const MovementIndicator = ({ value }) => {
    ...
    if (isSurge) return <span className="text-emerald-600 font-bold text-xs flex items-center bg-emerald-50 px-1 rounded border border-emerald-100">⚡ SURGE (+{value.toFixed(0)})</span>;
    if (isPlunge) return <span className="text-rose-600 font-bold text-xs flex items-center bg-rose-50 px-1 rounded border border-rose-100">⚡ PLUNGE ({value.toFixed(0)})</span>;
