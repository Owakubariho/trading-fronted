...
export const Cad3 = () => {
  ...
  const getStock = async () => {
    const response = await fetch("http://127.0.0.1:8000/Cad/");
  ...
  const CANADIANDOLLAR = data.find((item) => item.Asset === "CANADIAN DOLLAR");
  console.log(CANADIANDOLLAR);
  // Output:
  // {
  ...
  // }
  const commercialIndex = CANADIANDOLLAR?.CommercialWilliamsIndex;
  console.log(commercialIndex); // "71.41"
  const nonCommercialIndex = CANADIANDOLLAR?.NonCommercialWilliamsIndex;
        ...
        <div className="relative h-96">
          <Line
            ...
            options={{
              ...
              plugins: {
                title: {
                  text: "CANADIAN DOLLAR - CHICAGO MERCANTILE EXCHANGE",
                },
              },
              scales: {
                x: {
